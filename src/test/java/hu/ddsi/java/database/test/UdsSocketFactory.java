...
import eu.javaexperience.nativ.java.socket.NativeLinuxSocketImpl;
import eu.javaexperience.reflect.Mirror;

public class UdsSocketFactory implements SocketFactory, SocketMetadata {
		...
		public Socket connect(String hostname, int portNumber, Properties props)
			...
			if (props != null) {
				...
				String socketPath = props.getProperty("socket");
				if (this.host != null){
					...
					if(null != socketPath)
					{
						possibleAddresses = new InetAddress[]{null};
					}
					
					if (null != socketPath || !(wantsLocalBind || wantsTimeout || needsConfigurationBeforeConnect)) {
						...
						for (int i = 0; i < possibleAddresses.length; i++) {
							try {
								this.rawSocket = null != socketPath?createUdsSocket(socketPath):new Socket(possibleAddresses[i],
										port);
		...
		protected static Constructor<Socket> sockImpl = Mirror.getDeclaredConstructor(Socket.class, SocketImpl.class);
		protected static Method postAccept = Mirror.getDeclaredMethod(Socket.class, "postAccept");
		
		protected Socket createUdsSocket(String path)
		{
			try
			{
				NativeLinuxSocketImpl nl = new NativeLinuxSocketImpl();
				Socket s = sockImpl.newInstance(nl);
				nl.create(true);
				SockAddr addr = SockAddr.fromUds(path);
				try
				{
					nl.connect(addr);
					postAccept.invoke(s);
	...
	{
		System.out.println(new hu.ddsi.java.database.test.UdsSocketFactory().toString());
		System.out.println("forName: "+Class.forName("hu.ddsi.java.database.test.UdsSocketFactory").newInstance());
	}
}
*/